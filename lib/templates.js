...
export function detectPreInfoCondition(alarms, state = {}, now = new Date()) {
    ...
    // Dedup auf srf_id passiert hier implizit durch das Set.
    ...
    // entfernt sein. Wir nehmen den Median der valid_from-Zeitpunkte, damit
    // einzelne früh anstehende Alarme die Vorinformation nicht blockieren.
    ...
    // Schritt 4: Erster Gewitter-Alarm = Bezugspunkt für die Text-Formulierung
    // ("Heute ab 22:00 Uhr", "Morgen ab 07:00 Uhr", "Donnerstag ab 13:00 Uhr").
    // target-date = lokaler Schweizer Tag dieses Zeitpunkts (für State-Dedup).
    const firstValidFrom = validFromTimes[0];
    const targetDate = swissDateKey(new Date(firstValidFrom));
...
/**
 * Formatiert den Bezugspunkt des ersten Gewitter-Alarms als natürliche
 * Sprach-Phrase, z.B.:
 *   DE: "Heute ab 22:00 Uhr" / "Morgen ab 07:00 Uhr" / "Donnerstag, 4. Juni ab 13:00 Uhr"
 *   FR: "Aujourd'hui dès 22:00" / "Demain dès 07:00" / "Jeudi 4 juin dès 13:00"
 *   IT: "Oggi dalle 22:00" / "Domani dalle 07:00" / "Giovedì 4 giugno dalle 13:00"
 *
 * Tag und Uhrzeit werden in lokaler Schweizer Zeit (Europe/Zurich) gerechnet —
 * unabhängig davon, dass die API valid_from in UTC liefert.
 */
function formatFirstAlarmLabel(firstValidFromMs, lang, now = new Date()) {
    const first = new Date(firstValidFromMs);
    const firstKey = swissDateKey(first);
    const todayKey = swissDateKey(now);
    const tomorrowKey = swissDateKey(new Date(now.getTime() + 24 * 60 * 60 * 1000));
    const time = swissTimeLabel(first);

    let dayLabel;
    if (firstKey === todayKey) {
        dayLabel = RELATIVE_DAY[lang].today;
    } else if (firstKey === tomorrowKey) {
        dayLabel = RELATIVE_DAY[lang].tomorrow;
    } else {
        // Konkretes Datum: "Donnerstag, 4. Juni" / "jeudi 4 juin" / "giovedì 4 giugno"
        try {
    ...
    return `${dayLabel} ${TIME_CONNECTOR[lang]} ${time}${TIME_SUFFIX[lang]}`;
}
 ...
 * alle Vorlauf-Alarme.
 */
export function buildPreInfoTweets({ firstValidFrom, regionCount, regionRatio }, now = new Date()) {
    const result = {};
    for (const lang of ['de', 'fr', 'it']) {
        const when = formatFirstAlarmLabel(firstValidFrom, lang, now);

...
export function detectPreInfoCondition(alarms, state = {}, now = new Date()) {
    ...
    // Schritt 4: target-date = Mehrheits-Datum (UTC) der valid_from-Zeitpunkte.
    const dateCounts = {};
    for (const t of validFromTimes) {
        const d = new Date(t).toISOString().slice(0, 10);
        dateCounts[d] = (dateCounts[d] || 0) + 1;
    }
    const targetDate = Object.entries(dateCounts).sort((a, b) => b[1] - a[1])[0][0];
